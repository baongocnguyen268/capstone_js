  ...
  for (let i = 0; i < data.length; i++) {
    ...
    contentHTML += `
      ...
      product.name
            ...
            <h3 class="text-lg font-bold text-gray-800 mb-1 truncate w-full">${
              product.name
            }</h3>
            <div class="flex flex-col gap-1 text-gray-600 text-sm w-full">
                <div><span class="font-medium">Màn hình:</span> ${
                  product.screen
                }</div>
                <div><span class="font-medium">Camera trước:</span> ${
                  product.frontCamera
                }</div>
                <div><span class="font-medium">Camera sau:</span> ${
                  product.backCamera
                }</div>
            </div>
            <div class="flex items-center justify-center gap-2 mt-2 mb-3 w-full">
                <span class="text-purple-600 font-extrabold text-xl">${Number(
                  product.price
                ).toLocaleString()}₫</span>
            </div>
            <button class="add-to-cart mt-auto bg-gradient-to-r from-purple-500 to-purple-700 text-white px-5 py-2 rounded-full font-semibold shadow-md hover:from-purple-600 hover:to-purple-800 transition-all duration-200" data-product-id="${
              product.id
            }">
  ...
  cart.items.forEach((item) => {
    const itemHTML = `
            <div class="flex items-center gap-4 mb-4 p-2 border-b">
                 <img src="../../asset/image/${item.product.img}" alt="${
      item.product.name
                ...
                <div class="flex-grow">
                    ...
                    <p class="text-xs text-gray-600">${
                      item.quantity
                    } x ${Number(item.product.price).toLocaleString()}₫</p>
                    <div class="flex items-center gap-2 mt-1">
                        <button class="quantity-change border rounded-full w-6 h-6 leading-none" data-product-id="${
                          item.product.id
                ...
                <button class="remove-from-cart text-red-500 hover:text-red-700" data-product-id="${
                  item.product.id
                }">
...
getEle("filterType").addEventListener("change", function () {
